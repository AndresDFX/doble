...
type StatePatch = { enabled?: boolean; draft_mode?: boolean; user_name?: string };
...
export async function registerStateRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.patch<{ Body: StatePatch }>("/api/state", async (req, reply) => {
    ...
    if (enabled === undefined && draft_mode === undefined && user_name === undefined) {
      reply.status(400);
      return { error: "no fields to update" };
    }
    return container.agentState.patch({ enabled, draft_mode, user_name });
  });
}
